...
shot_touches <- big_5_pos_teams %>%
  ...
  left_join(., big_5_shot_teams %>% select("id", c("npx_g_expected", "so_t_standard",
                                              "gls_standard", "so_t_standard", "npx_g_per_sh_expected", "dist_standard", "sh_per_90_standard")), by = "id")
```
...
calculate_means(shot_touches, "prg_r_receiving", "att_pen_touches")
...
adj_var(shot_touches, "npx_g_expected")

shot_touches %>%
  mutate(across(c("npx_g_expected", "att_pen_touches", "gls_standard",
                  "so_t_standard"), ~ .x / mins_per_90)) %>%
  ggplot(aes(npx_g_expected, dist_standard, color = so_t_standard)) +
  geom_point() +
  ...
  # geom_vline(xintercept = calculate_means(shot_touches, NULL)) +
  # geom_hline(yintercept = calculate_means(shot_touches, NULL, "dist_standard")) +
  geom_text_repel(aes(label = squad), max.overlaps = 5) +
...
per_90_adj(shot_touches, "npx_g_expected", NULL) 
