...
shot_touches %>% 
  mutate(across(c("npx_g_expected", "att_pen_touches"), ~ .x / mins_per_90)) %>% 
  ggplot(aes(npx_g_expected, att_pen_touches, color = comp)) +
  geom_point() +
  #geom_smooth(method = "lm", se = FALSE) +
  geom_vline(xintercept = calculate_means(shot_touches, "npx_g_expected", NULL)) +
  geom_hline(yintercept = calculate_means(shot_touches, "att_pen_touches", NULL)) +
  geom_text_repel(aes(label = squad), max.overlaps = 5) +
