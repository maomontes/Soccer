...
calculate_means(shot_touches, "npx_g_expected", NULL)


shot_touches %>%
  mutate(across(c("npx_g_expected", "so_t_standard"), ~ .x / mins_per_90)) %>% 
  ...
  #geom_hline(yintercept = calculate_means(shot_touches, NULL, "so_t_standard")) +
  geom_text_repel(aes(label = squad), max.overlaps = 5) +
